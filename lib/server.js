...
var browserLibrarySource = read(path.normalize(__dirname + '/../browser/client.min.js'), 'utf-8');
...
var Server = function(httpServer, config) {
  ...
  this.config = {
    ...
    relayType: config.relayType || 'default',
    pubSubClient: config.pubSubClient || null,
  ...
  this.config.relay = this.initRelay();
 ...
 */
Server.prototype.initRelay = function() {
  var relay = null;

  if (availableRelays.indexOf(this.config.relayType) !== -1) {
    Relay = require('./relays/' + this.config.relayType);
...
Server.prototype.attach = function() {
  this.pollTransport = new PollTransport(this.httpServer, this.config);
...
Server.prototype.handleRequest = function(request, response) {
  ...
  if (request.url.indexOf(this.config.clientPath) === -1) {
