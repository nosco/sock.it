...
Server.prototype.attach = function() {
  ...
  this.pollTransport.on('connection', this.emit.bind('connection'));

  this.webSocketTransport = new WebSocketTransport.Server(this.httpServer, this.config);
  this.webSocketTransport.on('connection', this.emit.bind('connection'));
 ...
 */
Server.prototype.serveClient = function(request, response) {
