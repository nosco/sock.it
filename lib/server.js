...
var Server = function(httpServer, config) {
  ...
  debug.todo('In server.js is it safe to remove "dead" clients after 2 mins? What about slow connections and when the system is pressured?');
...
Server.prototype.initRelay = function(name) {
  ...
  if(availableRelays.indexOf(name) !== -1) {
    ...
    setTimeout(relay.cleanup, (this.config.cleanupInterval * 1000));
  }
