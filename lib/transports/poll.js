 ...
 * @param  {object} response http response
 */
PollServer.prototype.serve$ = function(request, response) {
  // For now, just serve the file directly from disk
  //   anything else slows down the workflow

  var jQuerySource = read(path.normalize(__dirname + '../../../browser/jquery.min.js'), 'utf-8');
  var jQueryVersion = new Date().getTime();

  var ifNoneMatch = request.headers['if-none-match'];
  if(ifNoneMatch && ifNoneMatch === jQueryVersion) {
    response.writeHead(304);
    response.end();
    return;
  }
  ...
  // Yay node, express and whatever...
  // This is the ONLY way I got ETag to work.......
  response.setHeader('ETag', jQueryVersion);
  response.writeHead(200, { etag: jQueryVersion });
  response.end(jQuerySource);
};
