...
PollTransport.prototype.handlePollRequest = function(request, response, clientInfo, next) {
  ...
  if(this.connectionsByConnId[clientInfo.connId]) {
    ...
    pollConnection.initiateListeners();
    pollConnection._ping = new Date().getTime();
    ...
    var pollConnection = new PollConnection({
      ...
      request: request,
      response: response,
      upgradeReq: { headers: request.headers }
    });
...
PollTransport.prototype.cleanup = function() {
  var checkTime = new Date().getTime();

  for(var i in this.connectionsByConnId) {
    if((this.connectionsByConnId[i]._ping + (this.cleanupInterval * 1000)) < checkTime) {
      this.connectionsByConnId[i].emit('close');
      delete this.connectionsByConnId[i];
    }
  }
};
