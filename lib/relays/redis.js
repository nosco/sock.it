          ...
          this.redisClient.multi()
            ...
            .exec(function(err, res) {
              if(!err && res[0] && res[0].length) {
                this.clientsByConnId[connId].sendMessage(JSON.stringify(res[0]), function(err) {
                  if(err) {
                    for(var i=res[0].length ; i > 0 ; i--) {
                      this.redisClient.lpush('sock.it:sendQueue::'+connId, res[0][(i-1)]);
                    }
                  }
                }.bind(this));
