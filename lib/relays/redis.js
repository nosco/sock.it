...
RedisRelay.prototype.findAndSendMessage = function(connId) {
  // console.log('findAndSendMessage');
  ...
  if(this.clientsByConnId[connId] && this.clientsByConnId[connId].sendState === this.clientsByConnId[connId].WAITING) {
    ...
    this.lookupClient.lrange('sock.it:messages::'+connId, 0, 1000, function(err, res) {
      if(err) {
        ...
        for(var i in res) {
console.log('MESSAGE (on worker: '+process.env.WORKER+'):');
console.log(res[i]);
...
RedisRelay.prototype.handleRequest = function(request, response, cb) {
  ...
  if(request.headers.cookie) {
    ...
    connId = cookies[this.cookieName] ? cookies[this.cookieName].value : null;
  }
...
if(!process.env.WORKER) {
  console.log("\n\n" +
              "--------------------------------------------------------------------------\n" +
              " This is a message from Sock.it\n" +
              " You're trying to use the redis stateKeeper\n" +
              " This is not doable without a process.env.WORKER id on each child process!\n" +
              " Please take a look at the example/clustered.js source.\n\n" +
              " TIP: If you use the module multi-cluster, it will handle it for you.\n" +
              "--------------------------------------------------------------------------\n");

  throw Error("Unable to find process.env.WORKER");
}
